...
const QuickMessageDialog = ({ open, onClose, quickemessageId, reload }) => {
    ...
    const handleSaveQuickeMessage = async (values) => {
        ...
        const quickemessageData = { ...values, isMedia: true, mediaPath: attachment ? String(attachment.name).replace(/ /g, "_") : values.mediaPath ? String(values.mediaPath).split(/[\\/]/).pop().replace(/ /g, "_") : null };
